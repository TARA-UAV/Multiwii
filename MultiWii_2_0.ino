...
March  2012     V2.0
 ...
 SNR 20120320: ADC Sonar & I2C working
 SNR 20120403: Autonomous obstacle avoidance
 SNR 20130805: Added heading data to be sent to Axon; Line695=AxonHeading=heading-180
 SNR 20130813: Added MAG Declination in CONFIG and IMU
 SNR 20130911: Send heading data to Axon in Hi and Lo bytes
 SNR 20131124: Send RC Data to Axon
 SNR 20150129: Automatically active maghold when autonomous mode is activated
 In progress: Autonomous take-off and hover 
...
#define YAW        2
#define THROTTLE   3
#define AUX1       4 // SNR Autonomous check
#define AUX2       5
#define AUX3       6
#define AUX4       7
...
static uint16_t axonHeading = 0;    //SNR
static uint8_t  axonHeadingLo = 0, axonHeadingHi=0;
static uint16_t Tz = 0;    //SNR
static uint8_t  TzLo = 0, TzHi=0;
...
static int16_t  debug1,debug2,debug3,debug4; //debug2 = hold altitude; debug3 = estimated altitude

static int16_t  SonarAlt; //SNR
...
static int16_t autoCommand[5]; //SNR autonomous command data
...
void loop () {
  ...
  if (currentTime > rcTime ) { // 50Hz
    ...
    //SNR - Autonomous Axon
    #if AUTONOMOUS
      AXON_write();//SNR - Send data to Axon: axonHeading
      AXON_read();//SNR - Get data from Axon: autoCommand[ROLL,PITCH,THROTTLE]
      //debug4 = rcCommand[ROLL];
      //debug2 = autoCommand[ROLL];
      // Check if autonomous flight is currently enabled
      if (autoCommand[AUX1]==1) {  
        //if(magMode==0) {
        //    magMode=1;
        //    activate1[BOXMAG]=7;
        //}   
        
          // Supplement RC data with Axon command
          rcData[ROLL]+=autoCommand[ROLL];
          rcData[PITCH]+=autoCommand[PITCH];
          rcData[YAW]+=autoCommand[YAW];
          rcData[THROTTLE]+=autoCommand[THROTTLE];
      }
      //else { 
      //  if(magMode==1) {
      //        magMode=0;
...
//    #endif
    ...
    //SNR
     ...
     //debug1 = rcOptions[BOXMAG]; //zzz
      //debug2 = magMode; //zzz
      //debug3 = activate2[BOXMAG]; //zzz
      //debug4 = activate1[BOXMAG];  //zzz
    #if MAG
      if (rcOptions[BOXMAG]) {
        if (magMode == 0) {
          magMode = 1;
...
//        #endif
  ...
  computeIMU();
  //SNR - Calculate heading for Axon
  axonHeading = uint16_t(heading+180);//adding 180 to make heading +ve integer 0-360
  axonHeadingHi = (axonHeading  & 0xff00UL) >>  8;
  axonHeadingLo = (axonHeading & 0x00ffUL);
...
//  
  ...
  //**** PITCH & ROLL & YAW PID ****   
 // SNR: NB Change control here if needed 
  for(axis=0;axis<3;axis++) {
  ...
  mixTable();//motor[0],motor[1],motor[2],motor[3] values are calculated and saturated to 1000-2000
    // SNR:Calculating Tz = motor[0]+motor[1]+motor[2]+motor[3]
  motorsum = motor[0]+motor[1]+motor[2]+motor[3]; //all int16_t numbers
  motorsum = motorsum/4;//here we are /4 for 1000to250 PWM scaling
  motorsum = 0.1608*motorsum - (0.0001617*motorsum)*motorsum - 32.22;
  //Tz = Tz*cos(p)*cos(r); //moved to Axon side
  Tz = uint16_t(motorsum);// SNR Tz feedback to Axon in uint16_t format
  //TzHi = (Tz  & 0xff00UL) >>  8;
  //TzLo = (Tz & 0x00ffUL);
  // Send Tz to Axon code in Sensors
  writeServos();
  writeMotors();
}
