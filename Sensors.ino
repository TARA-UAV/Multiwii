...
//SNR
#if !defined(AXON_ADDRESS) 
  #define AXON_ADDRESS 0xC0
#endif
...
// ************************************************************************************************************
#if defined(AXON)
static uint8_t axonData[5]; //range 0 to 255 - can send -127 to 127

void AXON_read(){
  i2c_rep_start(AXON_ADDRESS+1);
  for(uint8_t i = 0; i < 4; i++)
    axonData[i]=i2c_readAck();
  axonData[4] = i2c_readNak();

// Autonomous Roll command from Axon  
  autoCommand[ROLL] = (int16_t)axonData[0]-100;
  if (autoCommand[ROLL] > 100)
    autoCommand[ROLL] = 100;
  else if (autoCommand[ROLL] < -100)
    autoCommand[ROLL] = -100;

// Autonomous Pitch command from Axon  
  autoCommand[PITCH] = (int16_t)axonData[1]-100;
  if (autoCommand[PITCH] > 100)
    autoCommand[PITCH] = 100;
  else if (autoCommand[PITCH] < -100)
    autoCommand[PITCH] = -100;

// Autonomous Yaw command from Axon  
  autoCommand[YAW] = (int16_t)axonData[2]-100;
      if (autoCommand[YAW] > 100)
    autoCommand[YAW] = 100;
  else if (autoCommand[YAW] < -100)
    autoCommand[YAW] = -100;
  
// Autonomous Throttle command from Axon  
  autoCommand[THROTTLE] = (int16_t)axonData[3]-100;
    if (autoCommand[THROTTLE] > 100)
    autoCommand[THROTTLE] = 100;
  else if (autoCommand[THROTTLE] < -100)
    autoCommand[THROTTLE] = -100;
    
// Autonomous Check input from Axon  
  autoCommand[AUX1] = axonData[4]; // Set by 'A'=1 and 'Z'=0 keypress
}

void AXON_write(){
  i2c_rep_start(AXON_ADDRESS);
  
  //---Send data divided by 10
  //i2c_write((uint8_t)(rcData[PITCH]/10));
  //i2c_write((uint8_t)(rcData[ROLL]/10));
  //i2c_write((uint8_t)(rcData[YAW]/10));
  //i2c_write((uint8_t)(rcData[THROTTLE]/10));
  
  //i2c_write((uint8_t)(axonHeading));
  i2c_write((uint8_t)(axonHeadingHi));
  i2c_write((uint8_t)(axonHeadingLo));
  //i2c_write((uint8_t)(TzHi));
  //i2c_write((uint8_t)(TzLo));  
  i2c_write((uint8_t)(Tz*10));  
  i2c_write((uint8_t)(rcData[PITCH]/10));
  i2c_write((uint8_t)(rcData[ROLL]/10));
  i2c_write((uint8_t)(rcData[YAW]/10));
  i2c_write((uint8_t)(rcData[THROTTLE]/10));
  //End of send data divided by 10-----
...
void Sonar_update() {
  //SonarAlt =  analogRead(A1);
    SonarAlt =  autoCommand[AUX1];
}
#endif
...
// ************************************************************************************************************
// I2C Compass HMC6352 //SNR
// ************************************************************************************************************
// I2C adress: 0x42 (8bit)
// ************************************************************************************************************
#if defined(HMC6352)
  #define MYMAG_ADDRESS 0x42 >> 1
  #define MYMAG_DATA_REGISTER 0x41
  static uint16_t myMagData[2];
  
  void MyMag_init() {
//    i2c_write(MYMAG_ADDRESS); 
//    delay(10);
//    i2c_write(0x47); 
//    delay(10);
//    i2c_write(0x74); 
...
//    i2c_write(0x52); 
//    delay(10);
    //i2c_writeReg(0x47, 0x74, 0x52); //{52)10 Hz measurement rate, periodic set/reset on, continuous mode 0x47+0x4E-axon  0x74+0x52
    //delay(50);
    //i2c_writeReg(0x47,0x4E, 0x00); //Heading mode
  }
  
  void MyMag_read(){
    i2c_rep_start(MYMAG_ADDRESS);
//    i2c_write(0x42); 
    i2c_write(0x41); 
//    i2c_write(0x00); 
    //i2c_rep_start(MYMAG_ADDRESS+1);  // I2C read direction => I2C address + 1
    
    i2c_stop();
    delay(100);
//    
    i2c_rep_start(MYMAG_ADDRESS);
//    i2c_write(0x43);         // Start multiple read at the reg register
//    delay(10);
    myMagData[0] = i2c_readAck();
    myMagData[1] = i2c_readNak();
    
    heading = myMagData[0]; //first byte is MSB heading
    //heading = heading << 8; //shift MSB byte to top of int
    //heading = heading + myMagData[1];	//add LSB byte (2nd byte) to bottom of int
    //heading = heading/10; 		// in xxx.x degrees format
